...
- **毒圈半径按等级模型计算**（不用 is_shrinking 拟合——该标记是累计的，Lv2 时会把 Lv1 的痕迹算进去）：
  - Lv1：从地图边缘（1600）收缩到 Lv1 安全区半径（670），由 countdown（总量 20）线性驱动
  - Lv2：**从 Lv1 安全区半径（670）开始**收缩到 Lv2 安全区半径（67）——不是从地图边缘
